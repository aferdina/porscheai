...
class ExperimentManager:
    ...
    def __init__(
        ...
        # autohpo/hyperparameter/usedparameter.yml
        ...
        self.game_config_path: str | None = None
        self.seed = seed
    ...
    def read_hyperparameters(self) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        if self.env_name in list(hyperparams_dict.keys()):
            ...
            raise ValueError(
                f"Hyperparameters not found for {self.algo}-{self.env_name} in {self.config}"
            )
    ...
    def _preprocess_hyperparams(  # noqa: C901
        ...
        if "game_config_path" in hyperparams:
            assert os.path.exists(
                hyperparams["game_config_path"]
            ), f"path {hyperparams['game_config_path']} does not exist"
            self.game_config_path = hyperparams["game_config_path"]
            del hyperparams["game_config_path"]
    ...
    def entry_point(env_id: str) -> str:
        try:
            ...
            return "Import not working- my bad"  # pytype: disable=module-attr
    ...
    def create_envs(
        ...
        if (
            "Neck" in self.env_name
            or self.is_robotics_env(self.env_name)
            or "parking-v0" in self.env_name
