...
class ReplaceString(Enum):
    features_extractor = "ft_ex"
    pi_feature_extractor = "pi_ft_ex"
    vf_feature_extractor = "vf_ft_ex"
    mlp_extractor = "mlp_ft_ex"


def replace_with_enum(string_to_replace: str) -> str:
    for member in ReplaceString:
        string_to_replace = string_to_replace.replace(
            str(member.name), str(member.value)
        )
...
USED_STATISTICS = [np.mean, np.std, np.median]
...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self):
        # only log on last timestep
        # TODO: Add Tensorboard callback here

        return True

    def _on_rollout_end(self):
        # Log Network weights
        _network_weights = self.model.get_parameters()
        network_weights = _network_weights["policy"]
        for name, layer in network_weights.items():
            self.logger.record(replace_with_enum(name), layer.detach().numpy())
